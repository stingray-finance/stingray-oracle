...
export class State implements StructClass { __StructClass = true as const;
 ...
 id: UID.bcs, oracle_queue: ID.bcs, guardian_queue: ID.bcs, on_demand_package_id: ID.bcs

}) };

 static fromFields( fields: Record<string, any> ): State { return State.reified( ).new( { id: decodeFromFields(UID.reified(), fields.id), oracleQueue: decodeFromFields(ID.reified(), fields.oracle_queue), guardianQueue: decodeFromFields(ID.reified(), fields.guardian_queue), onDemandPackageId: decodeFromFields(ID.reified(), fields.on_demand_package_id) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): State { if (!isState(item.type)) { throw new Error("not a State type");

 }

 return State.reified( ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), oracleQueue: decodeFromFieldsWithTypes(ID.reified(), item.fields.oracle_queue), guardianQueue: decodeFromFieldsWithTypes(ID.reified(), item.fields.guardian_queue), onDemandPackageId: decodeFromFieldsWithTypes(ID.reified(), item.fields.on_demand_package_id) } ) }
...
export class ON_DEMAND implements StructClass { __StructClass = true as const;
 ...
 static fromFields( fields: Record<string, any> ): ON_DEMAND { return ON_DEMAND.reified( ).new( { dummyField: decodeFromFields("bool", fields.dummy_field) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): ON_DEMAND { if (!isON_DEMAND(item.type)) { throw new Error("not a ON_DEMAND type");

 }

 return ON_DEMAND.reified( ).new( { dummyField: decodeFromFieldsWithTypes("bool", item.fields.dummy_field) } ) }
