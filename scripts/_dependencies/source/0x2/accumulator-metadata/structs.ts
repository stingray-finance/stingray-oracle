...
export function isMetadata(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V40}::accumulator_metadata::Metadata` + '<'); }
...
export function isMetadataKey(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V40}::accumulator_metadata::MetadataKey` + '<'); }
...
export class MetadataKey<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V40}::accumulator_metadata::MetadataKey`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = MetadataKey.$typeName; readonly $fullTypeName: `${typeof PKG_V40}::accumulator_metadata::MetadataKey<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = MetadataKey.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: MetadataKeyFields<T>, ) { this.$fullTypeName = composeSuiType( MetadataKey.$typeName, ...typeArgs ) as `${typeof PKG_V40}::accumulator_metadata::MetadataKey<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): MetadataKeyReified<ToPhantomTypeArgument<T>> { return { typeName: MetadataKey.$typeName, fullTypeName: composeSuiType( MetadataKey.$typeName, ...[extractType(T)] ) as `${typeof PKG_V40}::accumulator_metadata::MetadataKey<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: MetadataKey.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => MetadataKey.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => MetadataKey.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => MetadataKey.fromBcs( T, data, ), bcs: MetadataKey.bcs, fromJSONField: (field: any) => MetadataKey.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => MetadataKey.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => MetadataKey.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => MetadataKey.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => MetadataKey.fetch( client, T, id, ), new: ( fields: MetadataKeyFields<ToPhantomTypeArgument<T>>, ) => { return new MetadataKey( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 static get bcs() { return bcs.struct("MetadataKey", {
...
export function isOwner(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V40}::accumulator_metadata::Owner`; }
...
export class Owner implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V40}::accumulator_metadata::Owner`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Owner.$typeName; readonly $fullTypeName: `${typeof PKG_V40}::accumulator_metadata::Owner`; readonly $typeArgs: []; readonly $isPhantom = Owner.$isPhantom;

 readonly balances: ToField<Bag>; readonly owner: ToField<"address">

 private constructor(typeArgs: [], fields: OwnerFields, ) { this.$fullTypeName = composeSuiType( Owner.$typeName, ...typeArgs ) as `${typeof PKG_V40}::accumulator_metadata::Owner`; this.$typeArgs = typeArgs;

 this.balances = fields.balances;; this.owner = fields.owner; }

 static reified( ): OwnerReified { return { typeName: Owner.$typeName, fullTypeName: composeSuiType( Owner.$typeName, ...[] ) as `${typeof PKG_V40}::accumulator_metadata::Owner`, typeArgs: [ ] as [], isPhantom: Owner.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Owner.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Owner.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Owner.fromBcs( data, ), bcs: Owner.bcs, fromJSONField: (field: any) => Owner.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Owner.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Owner.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Owner.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Owner.fetch( client, id, ), new: ( fields: OwnerFields, ) => { return new Owner( [], fields ) }, kind: "StructClassReified", } }
 ...
 static get bcs() { return bcs.struct("Owner", {
...
export function isOwnerKey(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V40}::accumulator_metadata::OwnerKey`; }
