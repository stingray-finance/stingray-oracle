...
export function isDkgState(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V2}::validator_v2::DkgState`; }
...
export class DkgState implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V2}::validator_v2::DkgState`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = DkgState.$typeName; readonly $fullTypeName: `${typeof PKG_V2}::validator_v2::DkgState`; readonly $typeArgs: []; readonly $isPhantom = DkgState.$isPhantom;

 readonly id: ToField<UID>

 private constructor(typeArgs: [], fields: DkgStateFields, ) { this.$fullTypeName = composeSuiType( DkgState.$typeName, ...typeArgs ) as `${typeof PKG_V2}::validator_v2::DkgState`; this.$typeArgs = typeArgs;

 this.id = fields.id; }

 static reified( ): DkgStateReified { return { typeName: DkgState.$typeName, fullTypeName: composeSuiType( DkgState.$typeName, ...[] ) as `${typeof PKG_V2}::validator_v2::DkgState`, typeArgs: [ ] as [], isPhantom: DkgState.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => DkgState.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => DkgState.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => DkgState.fromBcs( data, ), bcs: DkgState.bcs, fromJSONField: (field: any) => DkgState.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => DkgState.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => DkgState.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => DkgState.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => DkgState.fetch( client, id, ), new: ( fields: DkgStateFields, ) => { return new DkgState( [], fields ) }, kind: "StructClassReified", } }
