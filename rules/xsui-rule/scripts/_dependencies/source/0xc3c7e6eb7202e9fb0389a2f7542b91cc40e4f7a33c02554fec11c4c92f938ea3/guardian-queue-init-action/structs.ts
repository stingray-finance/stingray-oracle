...
export class GuardianQueueCreated implements StructClass { __StructClass = true as const;
 ...
 queue_id: ID.bcs, queue_key: bcs.vector(bcs.u8())

}) };

 static fromFields( fields: Record<string, any> ): GuardianQueueCreated { return GuardianQueueCreated.reified( ).new( { queueId: decodeFromFields(ID.reified(), fields.queue_id), queueKey: decodeFromFields(reified.vector("u8"), fields.queue_key) } ) }
 ...
 return GuardianQueueCreated.reified( ).new( { queueId: decodeFromFieldsWithTypes(ID.reified(), item.fields.queue_id), queueKey: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.queue_key) } ) }
