...
export class QueueAuthorityUpdated implements StructClass { __StructClass = true as const;
 ...
 queue_id: ID.bcs, existing_authority: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), }), new_authority: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), })

}) };

 static fromFields( fields: Record<string, any> ): QueueAuthorityUpdated { return QueueAuthorityUpdated.reified( ).new( { queueId: decodeFromFields(ID.reified(), fields.queue_id), existingAuthority: decodeFromFields("address", fields.existing_authority), newAuthority: decodeFromFields("address", fields.new_authority) } ) }
 ...
 return QueueAuthorityUpdated.reified( ).new( { queueId: decodeFromFieldsWithTypes(ID.reified(), item.fields.queue_id), existingAuthority: decodeFromFieldsWithTypes("address", item.fields.existing_authority), newAuthority: decodeFromFieldsWithTypes("address", item.fields.new_authority) } ) }
