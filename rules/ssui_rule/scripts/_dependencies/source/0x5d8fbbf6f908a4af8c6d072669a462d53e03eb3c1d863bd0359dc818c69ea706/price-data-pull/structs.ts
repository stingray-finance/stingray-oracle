...
export function isPriceData(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V2}::price_data_pull::PriceData`; }
...
export class PriceData implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V2}::price_data_pull::PriceData`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = PriceData.$typeName; readonly $fullTypeName: `${typeof PKG_V2}::price_data_pull::PriceData`; readonly $typeArgs: []; readonly $isPhantom = PriceData.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: PriceDataFields, ) { this.$fullTypeName = composeSuiType( PriceData.$typeName, ...typeArgs ) as `${typeof PKG_V2}::price_data_pull::PriceData`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): PriceDataReified { return { typeName: PriceData.$typeName, fullTypeName: composeSuiType( PriceData.$typeName, ...[] ) as `${typeof PKG_V2}::price_data_pull::PriceData`, typeArgs: [ ] as [], isPhantom: PriceData.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => PriceData.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => PriceData.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => PriceData.fromBcs( data, ), bcs: PriceData.bcs, fromJSONField: (field: any) => PriceData.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => PriceData.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => PriceData.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => PriceData.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => PriceData.fetch( client, id, ), new: ( fields: PriceDataFields, ) => { return new PriceData( [], fields ) }, kind: "StructClassReified", } }
