...
export class AggregatorAuthorityUpdated implements StructClass { __StructClass = true as const;
 ...
 aggregator_id: ID.bcs, existing_authority: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), }), new_authority: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), })

}) };

 static fromFields( fields: Record<string, any> ): AggregatorAuthorityUpdated { return AggregatorAuthorityUpdated.reified( ).new( { aggregatorId: decodeFromFields(ID.reified(), fields.aggregator_id), existingAuthority: decodeFromFields("address", fields.existing_authority), newAuthority: decodeFromFields("address", fields.new_authority) } ) }
 ...
 return AggregatorAuthorityUpdated.reified( ).new( { aggregatorId: decodeFromFieldsWithTypes(ID.reified(), item.fields.aggregator_id), existingAuthority: decodeFromFieldsWithTypes("address", item.fields.existing_authority), newAuthority: decodeFromFieldsWithTypes("address", item.fields.new_authority) } ) }
