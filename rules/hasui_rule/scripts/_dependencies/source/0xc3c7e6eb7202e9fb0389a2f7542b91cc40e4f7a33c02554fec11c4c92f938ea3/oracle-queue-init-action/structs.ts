...
export class OracleQueueCreated implements StructClass { __StructClass = true as const;
 ...
 queue_id: ID.bcs, guardian_queue_id: ID.bcs, queue_key: bcs.vector(bcs.u8())

}) };

 static fromFields( fields: Record<string, any> ): OracleQueueCreated { return OracleQueueCreated.reified( ).new( { queueId: decodeFromFields(ID.reified(), fields.queue_id), guardianQueueId: decodeFromFields(ID.reified(), fields.guardian_queue_id), queueKey: decodeFromFields(reified.vector("u8"), fields.queue_key) } ) }
 ...
 return OracleQueueCreated.reified( ).new( { queueId: decodeFromFieldsWithTypes(ID.reified(), item.fields.queue_id), guardianQueueId: decodeFromFieldsWithTypes(ID.reified(), item.fields.guardian_queue_id), queueKey: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.queue_key) } ) }
