...
export class AggregatorDeleted implements StructClass { __StructClass = true as const;
 ...
 aggregator_id: ID.bcs

}) };

 static fromFields( fields: Record<string, any> ): AggregatorDeleted { return AggregatorDeleted.reified( ).new( { aggregatorId: decodeFromFields(ID.reified(), fields.aggregator_id) } ) }
 ...
 return AggregatorDeleted.reified( ).new( { aggregatorId: decodeFromFieldsWithTypes(ID.reified(), item.fields.aggregator_id) } ) }
