...
export class Attestation implements StructClass { __StructClass = true as const;
 ...
 guardian_id: ID.bcs, secp256k1_key: bcs.vector(bcs.u8()), timestamp_ms: bcs.u64()

}) };

 static fromFields( fields: Record<string, any> ): Attestation { return Attestation.reified( ).new( { guardianId: decodeFromFields(ID.reified(), fields.guardian_id), secp256K1Key: decodeFromFields(reified.vector("u8"), fields.secp256k1_key), timestampMs: decodeFromFields("u64", fields.timestamp_ms) } ) }
 ...
 return Attestation.reified( ).new( { guardianId: decodeFromFieldsWithTypes(ID.reified(), item.fields.guardian_id), secp256K1Key: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.secp256k1_key), timestampMs: decodeFromFieldsWithTypes("u64", item.fields.timestamp_ms) } ) }
...
export class Oracle implements StructClass { __StructClass = true as const;
 ...
 id: UID.bcs, oracle_key: bcs.vector(bcs.u8()), queue: ID.bcs, queue_key: bcs.vector(bcs.u8()), expiration_time_ms: bcs.u64(), mr_enclave: bcs.vector(bcs.u8()), secp256k1_key: bcs.vector(bcs.u8()), valid_attestations: bcs.vector(Attestation.bcs), version: bcs.u8()

}) };

 static fromFields( fields: Record<string, any> ): Oracle { return Oracle.reified( ).new( { id: decodeFromFields(UID.reified(), fields.id), oracleKey: decodeFromFields(reified.vector("u8"), fields.oracle_key), queue: decodeFromFields(ID.reified(), fields.queue), queueKey: decodeFromFields(reified.vector("u8"), fields.queue_key), expirationTimeMs: decodeFromFields("u64", fields.expiration_time_ms), mrEnclave: decodeFromFields(reified.vector("u8"), fields.mr_enclave), secp256K1Key: decodeFromFields(reified.vector("u8"), fields.secp256k1_key), validAttestations: decodeFromFields(reified.vector(Attestation.reified()), fields.valid_attestations), version: decodeFromFields("u8", fields.version) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Oracle { if (!isOracle(item.type)) { throw new Error("not a Oracle type");

 }

 return Oracle.reified( ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), oracleKey: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.oracle_key), queue: decodeFromFieldsWithTypes(ID.reified(), item.fields.queue), queueKey: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.queue_key), expirationTimeMs: decodeFromFieldsWithTypes("u64", item.fields.expiration_time_ms), mrEnclave: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.mr_enclave), secp256K1Key: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.secp256k1_key), validAttestations: decodeFromFieldsWithTypes(reified.vector(Attestation.reified()), item.fields.valid_attestations), version: decodeFromFieldsWithTypes("u8", item.fields.version) } ) }
