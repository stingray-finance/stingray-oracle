...
export class QueueConfigsUpdated implements StructClass { __StructClass = true as const;
 ...
 queue_id: ID.bcs, name: String.bcs, fee: bcs.u64(), fee_recipient: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), }), min_attestations: bcs.u64(), oracle_validity_length_ms: bcs.u64()

}) };

 static fromFields( fields: Record<string, any> ): QueueConfigsUpdated { return QueueConfigsUpdated.reified( ).new( { queueId: decodeFromFields(ID.reified(), fields.queue_id), name: decodeFromFields(String.reified(), fields.name), fee: decodeFromFields("u64", fields.fee), feeRecipient: decodeFromFields("address", fields.fee_recipient), minAttestations: decodeFromFields("u64", fields.min_attestations), oracleValidityLengthMs: decodeFromFields("u64", fields.oracle_validity_length_ms) } ) }
