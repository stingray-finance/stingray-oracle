...
export class OracleCreated implements StructClass { __StructClass = true as const;
 ...
 oracle_id: ID.bcs, queue_id: ID.bcs, oracle_key: bcs.vector(bcs.u8())

}) };

 static fromFields( fields: Record<string, any> ): OracleCreated { return OracleCreated.reified( ).new( { oracleId: decodeFromFields(ID.reified(), fields.oracle_id), queueId: decodeFromFields(ID.reified(), fields.queue_id), oracleKey: decodeFromFields(reified.vector("u8"), fields.oracle_key) } ) }
 ...
 return OracleCreated.reified( ).new( { oracleId: decodeFromFieldsWithTypes(ID.reified(), item.fields.oracle_id), queueId: decodeFromFieldsWithTypes(ID.reified(), item.fields.queue_id), oracleKey: decodeFromFieldsWithTypes(reified.vector("u8"), item.fields.oracle_key) } ) }
